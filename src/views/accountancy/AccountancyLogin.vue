      ...
      <div class="container">
        <div class="form">
          ...
          <div class="form-group submit">
            <!-- <RouterLink to="accountancy" class="btn-style medium round">Login</RouterLink> -->
            <b-button
              variant=""
...
/* eslint-disable */
// @ is an alias to /src
import { AccountancyLogin } from "../../Store";
...
export default {
  ...
  methods: {
    InitializeLoginAction() {
      ...
      AccountancyLogin(this.formData)
        .then((response) => {
          this.isLoading = true;
          const notyf = new Notyf({duration: 4000,position: { x: "right", y: "top" },});
          localStorage.setItem("token", response.data.token);
          notyf.success({
                    message: `Auth : ${response.data.message}`,
                });
          setTimeout(() => {
                 this.isLoading = false;
